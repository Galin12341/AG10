...
flake8 app.py config.py utils.py --statistics --count > flake8_report.txt 2>&1 || true
echo "✓ Flake8 掃描完成 - 結果保存至 flake8_report.txt"
echo

echo "[3/6] 運行 Pylint (代碼品質分析)..."
pylint app.py config.py utils.py --output-format=text > pylint_report.txt 2>&1 || true
echo "✓ Pylint 掃描完成 - 結果保存至 pylint_report.txt"
echo

echo "[4/6] 運行 Safety (依賴漏洞掃描)..."
safety check -r requirements.txt --output text > safety_report.txt 2>&1 || true
echo "✓ Safety 掃描完成 - 結果保存至 safety_report.txt"
echo

echo "[5/6] 運行 Semgrep (SAST 安全分析)..."
semgrep --config .semgrep.yml app.py config.py utils.py > semgrep_simple_report.txt 2>&1 || true
echo "✓ Semgrep 掃描完成 - 結果保存至 semgrep_simple_report.txt"
echo

echo "[6/6] 運行 Bandit (Python 安全掃描)..."
bandit -r . -f txt -o bandit_report.txt 2>&1 || true
